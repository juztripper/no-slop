...
`src/background` is a Manifest V3 service worker. It validates content messages, enforces first-run consent, domain exceptions, enabled platforms, top-frame-only analysis, and known private-page exclusions. It serializes settings saves, keeps tokens in trusted local extension storage, redacts tokens from content-script settings, and stores tab counters and request-pacing metadata in session storage. It aborts outstanding requests when settings change or a tab navigates. Credentials go only to the configured HTTPS or loopback detector; redirects are forbidden for that request.

Analysis requests share a two-second admission interval across tabs (up to 30 batches per minute, below the default service limit of 40). The next admission time and endpoint are stored in trusted session storage so a service-worker restart does not reset pacing. A deferred batch returns immediately to the content script rather than holding a worker fetch open. HTTP 429 responses extend the shared cooldown using `Retry-After` (seconds or an HTTP date; 61 seconds when absent). Content remains visible and the popup shows “Resuming shortly…”; a timer re-extracts current visible items and retries automatically. Pause, consent revocation, restore, navigation and disposal cancel page retries. Non-retryable failures still surface as errors; daily provider-call limits are unchanged.
